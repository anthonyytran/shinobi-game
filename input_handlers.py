...
"""
An event handler returns either an action or another event handler.
...
class EventHandler(BaseEventHandler):
    ...
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        ...
        if self.handle_action(action_or_state):
            # A valid was performed. 
            if not self.engine.player.is_alive:
                # The player was killed sometime during or after this action.
...
class InventoryActivateHandler(InventoryEventHandler):
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        """Return the action for selected item."""
...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key == tcod.event.KeySym.K_PERIOD and modifier & (
